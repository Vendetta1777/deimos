...
You are setting up a local voice assistant ("Jarvis") on a Mac (Apple Silicon
...
- The newest, correct project files are in THIS folder (the one containing this
  file: server.py, main.py, requirements.txt, web/, and the jarvis/ package).
- The user may already have older copies at ~/jarvis and/or ~/Desktop/jarvis.
...
- Long-term memory lives at ~/.jarvis/memory.db (OUTSIDE the project). Never
  delete this; it is the user's accumulated memory.

## Goal
A single project folder at ~/jarvis that contains THESE (newest) files, plus a
working .venv and the voices/ folder, with no leftover duplicate on the Desktop,
and the server running.

## Steps
1. Identify all existing jarvis copies: check ~/jarvis and ~/Desktop/jarvis.
   Report what each contains (especially .venv and voices/).

2. Establish ~/jarvis as the single home for the project:
   - Copy ALL files from this folder into ~/jarvis, OVERWRITING older versions.
     (These files are the source of truth. Overwrite server.py, main.py,
     requirements.txt, README.md, and the entire web/ and jarvis/ trees.)
   - If a .venv already exists in ~/jarvis OR ~/Desktop/jarvis, reuse the
     existing one (move it into ~/jarvis if needed). Otherwise create a new
     venv: `python3 -m venv .venv`.
   - If a voices/ folder with the .onnx model exists anywhere, make sure it
     ends up at ~/jarvis/voices/. Otherwise leave it; the app falls back to a
     system voice.
...
4. Remove the duplicate ONLY after ~/jarvis is confirmed complete and working:
   delete ~/Desktop/jarvis. Do NOT touch ~/.jarvis (memory).

5. Verify the version is the NEWEST UI before declaring success:
   `grep -c textin ~/jarvis/web/index.html` MUST print 0. (If it prints 1+, the
   old files are still in place — re-copy from this folder.)

6. Launch: from ~/jarvis with the venv active, run `python server.py`.
...
## Voice expectation
With the Piper model at ~/jarvis/voices/en_GB-alan-medium.onnx and piper-tts
